"""
Usage:
train.py <trainpath> <labelpath> <name> <savedir> [-v]
...
def main(args):
    ...
    #train_path = "/Data/train_rois4.npy"
    #label_path = "/Data/train_labels.csv"
    #savedir = "../Model outputs/"
    
    
    #Load and format data
    label_encoder = LabelEncoder()
    train_data, labels = load_data(train_path=train_path,label_path=label_path, label_encoder= label_encoder)
    ...
    if validate: 
        net = NeuralNetClassifier(module=cnn.KaggleNet().double(), criterion=nn.CrossEntropyLoss, optimizer = optim.Adam, 
                                lr= 0.00001, max_epochs = 40, train_split = CVSplit(0.1, random_state = 0)
                                , device = device, iterator_train__batch_size = 128, iterator_valid__batch_size=512)
    else:
        net = NeuralNetClassifier(module=cnn.KaggleNet().double(), criterion=nn.CrossEntropyLoss, optimizer = optim.Adam, 
                                lr= 0.00001, max_epochs = 40, train_split = None
                                ,device = device, iterator_train__batch_size = 128, iterator_valid__batch_size=512)
    ...
    #Save the model
    net.save_params(f_params=savedir+'model_params_'+name+'.pkl', 
            f_history=savedir+'model_history_'+name+'.pkl', f_optimizer=savedir+'model_optimizer_'+name+'.pkl')
